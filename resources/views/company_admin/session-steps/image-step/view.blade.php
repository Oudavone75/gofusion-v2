            ...
            <div class="page-header">
                <div class="row">
                    @include('company_admin.components.page-title', [
                        'page_title' => 'View Challenges to Complete Step',
                        'paths' => breadcrumbs(),
                    ])
                </div>
            </div>
            <div class="pd-20 card-box mb-30">
                <div class="row">
                    ...
                    <div class="col-md-6 col-sm-12 mt-2 ml-0">
                        ...
                        <p class="form-control-plaintext">
                            {{ $image_step->goSessionStep->goSession->campaignSeason->department->name ?? 'Not Available' }}
                        </p>
                    </div>
                    <div class="col-md-6 col-sm-12 mt-2 ml-0">
                                ...
                                @if (!empty($image_step->keywords) && count($image_step->keywords) > 0)
                                    @foreach ($image_step->keywords as $keyword)
                                        <span class="badge badge-primary mr-1 mb-1"
                                            style="font-size: 14px;">{{ $keyword }}</span>
                                    @endforeach
                                @else
                                    —
                                @endif
