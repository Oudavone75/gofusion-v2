                        ...
                        <table class="table table-hover nowrap">
                            <thead>
                                <tr>
                                    ...
                                    @if (auth('web')->user()->hasDirectPermission('manage posts reports'))
                                        <th>Action</th>
                                    @else
                                        <th>Status</th>
                                    @endif
                            ...
                            <tbody>
                                @forelse ($reported_posts as $post)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('company_admin.social-feed.view', $post->id) }}"
                                                class="text-primary font-weight-bold" target="_blank">
                                                View Post
                                            </a>
                                        </td>
                                        <td>
                                            @if (auth('web')->user()->hasDirectPermission('view reported users'))
                                            <a class="badge badge-primary"
                                                href="{{ $post->reports_count > 0 ? route('company_admin.social-feed.reported-users-list', $post->id) : '#' }}">
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right dropdown-menu-icon-list">
                                                    <button class="dropdown-item text-success"
                                                        onClick="updateReportStatus('{{ route('company_admin.social-feed.reports.status', [$post->id, 'resolved']) }}')">
                                                        <i class="dw dw-check"></i> Approve
                                                    </button>
                                                    <button class="dropdown-item text-danger"
                                                        onClick="updateReportStatus('{{ route('company_admin.social-feed.reports.status', [$post->id, 'dismissed']) }}')">
