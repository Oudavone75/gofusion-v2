    ...
    <div class="pd-ltr-20 xs-pd-20-10">
        <div class="min-height-200px">
            <div class="page-header">
                <div class="row">
                    @include('admin.components.page-title', ['page_title' => 'Dashboard'])
                </div>
            </div>
            <div class="mb-30">
                <div class="row">
                            ...
                            <div class="col-12">
                                <a href="{{ route('admin.company.index') }}">
                                    <div class="stats-card stats-card-secondary">
                                        <div class="stats-card-body">
                                            <div class="stats-header">
                                                <span class="stats-title">Total Companies</span>
                                                <div class="stats-icon">
                                                    <i class="micon dw dw-building1"></i>
                                                </div>
                                            </div>
                                            <div class="stats-number">{{ $companyCount ?? 0 }}</div>
                                        </div>
                                    </div>
                                </a>
                            </div>
                            ...
                            <div class="col-12 mb-3">
                                <a href="{{ route('admin.citizens.index') }}">
                                    <div class="stats-card stats-card-quaternary">
                                        <div class="stats-card-body">
                                            <div class="stats-header">
                                                <span class="stats-title">Total Citizens</span>
                                                <div class="stats-icon">
                                                    <i class="micon icon-copy dw dw-user2"></i>
                                                </div>
                                            </div>
                                            <div class="stats-number">{{ $citizenCount ?? 0 }}</div>
                                        </div>
                                    </div>
                                </a>
                            </div>
            ...
            @if ($challenges->count() > 0)
                ...
                <div class="card-box mb-30">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if (auth('admin')->user()->hasDirectPermission('manage inspiration challenges user requests'))
                                                    <!-- Accept Button -->
                                                    <form
                                                        action="{{ route('admin.inspiration-challenges.pending.status', [$challenge->id, 'accept']) }}"
                    ...
                    <div class="text-center pb-20">
                        <a href="{{ route('admin.inspiration-challenges.pending') }}" class="btn btn-primary">
            ...
            @elseif($campaigns->count() > 0)
                <div class="page-header">
                    <div class="row">
                        @include('admin.components.page-title', ['page_title' => 'Seasons'])
                    </div>
                </div>
                <div class="card-box mb-30">
                    <div class="pb-20">
                    ...
                    <div class="text-center pb-20">
                        <a href="{{ route('admin.campaign.index', ['type' => 'season']) }}" class="btn btn-primary">
                            <i class="micon dw dw-eye"></i> View All Seasons
                        </a>
                    </div>
                </div>
            @else
        ...
        @if(isset($allCampaigns) && $allCampaigns->count() > 0)
            @include('admin.partials.performance-dashboard')
        @endif
        </div>
    </div>
@endsection
...
$(document).ready(function () {
    ...
    var donutChart = null;
    var currentPage = 1;
    var statsUrl = '{{ route("admin.performance.dashboard-stats") }}';
    var employeeDetailBaseUrl = '{{ route("admin.performance.employee-detail", ":userId") }}';

    // Campaign and Season data from server
    var campaignsList = @json($campaignsJson);
    var seasonsList = @json($seasonsJson);
    ...
    function populateCampaignDropdown(type) {
        var list = type === 'campaign' ? campaignsList : seasonsList;
        var $select = $('#performanceCampaignFilter');
        $select.empty();
        ...
        $.each(list, function (i, c) {
            $select.append(
                $('<option>', {
                    value: c.id,
                    text: c.title + ' (' + formatDate(c.start_date) + ' - ' + formatDate(c.end_date) + ')'
                })
            );
        });
        ...
        // Load stats for the first item
        var firstId = $select.val();
        if (firstId) {
            loadPerformanceStats(firstId, 1);
        } else {
    ...
    // Type filter change — repopulate campaign dropdown
    $('#performanceTypeFilter').on('change', function () {
        populateCampaignDropdown($(this).val());
    });
    ...
    // Initial load with default type 'campaign'
